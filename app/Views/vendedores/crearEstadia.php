...
<div class="container">
    <form style="width: 50%; margin: 0 auto" id="formulario" method="post" action="<?= base_url('usuarios/vendedores/vender') ?>">
...
<script type="text/javascript">
    ...
    const crearHorarios = (horario) => {
        if (horario > 0)
        {
            fetch("<?= esc(base_url('usuarios/vendedores/horarios')) ?>", {
                method: 'GET',
    ...
    const previsualizar = () => {
        ...
        fetch("<?= esc(base_url('usuarios/vendedores/precio')) ?>", {
            method: 'GET',
    ...
    fetch("<?= esc(base_url('usuarios/vendedores/zonas')) ?>", { method: 'GET' })
        .then(response => response.json())
        .then(data => {
